    ...
    const newTradeList = await Promise.all(
      tradeList.map(async (trade) => {
        ...
        if (sell_open && buy_quantity > 0) {
          ...
          if (sell_history_trade.length === 0) {
            log(`币种${symbol} 没有达到条件卖出`)
          }
